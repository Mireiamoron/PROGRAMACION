...
public class RETO_LOTERIA {
	...
	// devuelve un numero al azar del bombo de números, que será objeto de un
	// premio.
	...
	// Comprueba si un décimo ha sido agraciado y en tal caso indica por consola el
	// premio
	public static String heSidoAgraciado(String[] numerosSorteo, String[] premiosSorteo, String miDecimo,
			String numeroPrimerPremio, String numeroSegundoPremio, String numeroTercerPremio, String primerosCuartosPremios, String numeroQuintosPremios) {
		String premio = "Número no premiado"; // valor por defecto

		// Comprueba premios principales
		...
		// Mireia
		if (miDecimo.substring(3).equals(numeroPrimerPremio.substring(3))) {
			premio = "Premio: Dos últimas cifras del primer premio (120 euros por décimo)";
		}
		// Mireia

		if (miDecimo.substring(3).equals(numeroSegundoPremio.substring(3))) {
			premio = "Premio: Dos últimas cifras del segundo premio (100 euros por décimo)";
		}

		if (miDecimo.substring(3).equals(numeroTercerPremio.substring(3))) {
			premio = "Premio: Dos últimas cifras del tercer premio (100 euros por décimo)";
		}
		
		if (miDecimo.substring(3).equals(primerosCuartosPremios.substring(3))) {
			premio = "Premio: Tres primeras cifras de los primeros cuartos premios)";
			
		}
		
		if (miDecimo.substring(3).equals(numeroQuintosPremios.substring(3))) {
			premio = "Premio: Un euro por cada uno jugado(20 euros por décimo)";
		}

		return premio;
	}

	// Implementa un bucle para comprobar, haciendo uso del método heSidoAgraciado,
	// si nuestro décimo tiene o no premio
	public static void compruebaDecimos(String[] numerosSorteo, String[] premiosSorteo, String numeroPrimerPremio,
			String numeroSegundoPremio, String numeroTercerPremio, String primerosCuartosPremios, String numeroQuintosPremios) throws ElGordoException {
		...
		do {
			...
			if (!terminarDeComprobar)
				miPremio = heSidoAgraciado (numerosSorteo, premiosSorteo, miDecimo, numeroPrimerPremio,
						numeroSegundoPremio, numeroTercerPremio,primerosCuartosPremios, numeroQuintosPremios);
	...
	public static void main(String[] args) {
		...
		String numeroPrimerPremio = ""; // Guardamos el número del primer premio
		String numeroSegundoPremio = ""; // Guardamos el número del segundo premio
		String numeroTercerPremio = ""; // Guardamos el número del tercer premio
		String primerosCuartosPremios =""; // CENTENAS
		String numeroQuintosPremios =""; // REINTEGRO DEL PRIMER PREMIO
		try {
			...
			for (int i = 0; i < MAX_PREMIOS; i++) {
				...
				if (premio.equals("PRIMER PREMIO")) {
					numeroPrimerPremio = numeroFormateado;
				}
				if (premio.equals("SEGUNDO PREMIO")) {
					numeroSegundoPremio = numeroFormateado;
				}
				if (premio.equals("TERCER PREMIO")) {
					numeroTercerPremio = numeroFormateado;
				}
			...
			compruebaDecimos(listaNumerosSorteo, listaPremiosSorteo, numeroPrimerPremio, numeroSegundoPremio,
					numeroTercerPremio, primerosCuartosPremios, numeroQuintosPremios);
