    ...
    public static void compruebaDecimos(String[] numerosSorteo, String[] premiosSorteo)
        ...
        while (true) {
            ...
            public static void main(String[] args) {
                ...
                final int MAX_NUMEROS = 100000;//numeros del sorteo(mmc)
                final int MAX_PREMIOS = 1807;//premios del sorteo(mmc)
                ...
                try {//creamos los bombos con sus bolas(mmc)
                    bomboBolas = creaBomboNumeros(MAX_NUMEROS);
                    bomboPremios = creaBomboPremios(MAX_PREMIOS);
                    //Mostramos el listado completo de números premiados(prr)
                    System.out.println("\nLISTADO OFICIAL DE PREMIOS:\n");
                    	int j= 0;
                    for (int i = 0; i < MAX_PREMIOS; i++) {
                    	//Sacamos un número del bombo y lo rellenamos con ceros a la izquierda(prr)
                    	//sacamos un premio del bombo
                    	////Añado el número a la lista oficial del sorteo en la posición j(prr)
                	...
                	//COMPLETAR (capturar excepción propia)(MMC)
            		
                    compruebaDecimos(listaNumerosSorteo, listaPremiosSorteo);

                } catch (GordoException ge) {
